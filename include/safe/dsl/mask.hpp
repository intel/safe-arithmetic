...
template <auto VariableBits, auto ConstantBits = 0>
constexpr mask_t<to_big_integer(VariableBits), to_big_integer(ConstantBits)>
    mask{};
...
template <auto min, auto max> struct to_ival<ival_t<min, max>> {
    using type = ival_t<min, max>;
};
...
template <auto min, auto max> struct to_mask<ival_t<min, max>> {
