...
template <char Char>
constexpr bool is_hex_char_v = (Char >= 'A' && Char <= 'F' ||
                                Char >= 'a' && Char <= 'f');

template <char Char>
constexpr bool is_hex_digit_v = is_decimal_digit_v<Char> ||
                                is_hex_char_v<Char>;

template <char Char0, char Char1, char... Chars>
concept hex_number = is_0_v<Char0> && is_x_v<Char1> &&
                     (is_hex_digit_v<Chars> && ...);

template <typename T, char... Chars>
concept hex_integer = std::unsigned_integral<T> && hex_number<Chars...>;
} // namespace safe::detail
