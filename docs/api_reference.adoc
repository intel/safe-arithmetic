...
using the `safe::match_constraint` function.

```c++
auto char_to_ord = safe::match_constraint(
...
`safe::match_constraint`. This is the power of `safe::match_constraint`: check runtime values are 
...
3. Use `safe::match_constraint` to conditionally assign the result.
...
// for (safe::ival_s32<0, 9> i = 0_cn; i < 10_cn; i = i + 1_cn) {
//     :'(
// }

for (auto i : safe::irange(0_cn, 10_cn)) {
