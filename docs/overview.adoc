...
through `constraint_cast<T>(value)`. It bypasses all compile-time and runtime
...
```c++
template<typename T>
T constraint_cast(auto value);
```

`constraint_cast<T>(value)` is used within the Safe Arithmetic library to ferry
...
auto dangerous_value = constraint_cast<safe::ival_u32<0, 1024>>(some_function());
...
// SAFE: Don't use constraint_cast<T>(value), try almost everything else first.
```
