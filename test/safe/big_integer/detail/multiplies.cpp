...
namespace safe::_big_integer::detail {
    ...
    RC_GTEST_PROP(multiplies, asymmetric_is_commutative, (storage<256> a, storage<128> b)) {
        is_commutative(multiplies, a, b);
    }
    ...
    RC_GTEST_PROP(multiplies, is_associative_32, (storage<32> a, storage<32> b, storage<32> c)) {
        is_associative<storage<96>>(multiplies, a, b, c);
    }

    RC_GTEST_PROP(multiplies, is_associative_64, (storage<64> a, storage<64> b, storage<64> c)) {
        is_associative<storage<192>>(multiplies, a, b, c);
    }

    RC_GTEST_PROP(multiplies, is_associative_128, (storage<128> a, storage<128> b, storage<128> c)) {
    ...
    RC_GTEST_PROP(multiplies, is_multiply_64, (int64_t a, int64_t b)) {
        storage<64> actual{}; multiplies(actual, make_storage(a), make_storage(b));
    ...
    RC_GTEST_PROP(multiplies, is_multiply_32, (int32_t a, int32_t b)) {
        storage<64> actual{}; multiplies(actual, make_storage(a), make_storage(b));
        storage<64> expected = make_storage(static_cast<int64_t>(a) * static_cast<int64_t>(b));
        RC_ASSERT(actual == expected);
    }
    ...
    RC_GTEST_PROP(multiplies, shift_and_multiply_equivalence, (storage<512> a)) {
        ...
        storage<1024> shift_result{}; bit_shift_left(shift_result, a, i);

        storage<1024> shifted_one{}; bit_shift_left(shifted_one, make_storage(1), i);
        storage<1024> mul_result{}; multiplies(mul_result, shifted_one, a);
