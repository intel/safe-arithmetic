...
template <std::size_t NumBits> void is_idempotent(auto op, storage<NumBits> a) {
    storage<NumBits> result{};
    op(result, a, a);
    RC_ASSERT(a == result);
}
...
template <typename ResultType> void lhs_can_be_result(auto op, auto a, auto b) {
    ResultType result0{};
    op(result0, a, b);
    ResultType result1{a};
    op(result1, result1, b);
    RC_ASSERT(result0 == result1);
}

template <typename T> constexpr auto negation_will_not_overflow(T a) -> bool {
    return a != std::numeric_limits<T>::min();
}

template <typename T>
constexpr auto addition_will_not_overflow(T a, T b) -> bool {
    return std::signbit(a) != std::signbit(b) or
           (a >= 0 and a <= std::numeric_limits<T>::max() - b) or
           (a < 0 and a >= std::numeric_limits<T>::min() - b);
}

template <typename T>
constexpr auto subtraction_will_not_overflow(T a, T b) -> bool {
    return std::signbit(a) == std::signbit(b) or
           (a >= 0 and a <= std::numeric_limits<T>::max() + b) or
           (a < 0 and a >= std::numeric_limits<T>::min() + b);
}
