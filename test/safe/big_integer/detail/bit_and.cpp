...
RC_GTEST_PROP(bit_and, asymmetric_is_commutative,
              (storage<128> a, storage<64> b)) {
    is_commutative(bit_and, a, b);
}
...
RC_GTEST_PROP(bit_and, is_associative,
...
RC_GTEST_PROP(bit_and, is_and, (int64_t a, int64_t b)) {
    storage<64> actual{};
    bit_and(actual, make_storage(a), make_storage(b));
