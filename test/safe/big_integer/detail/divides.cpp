...
namespace safe::_big_integer::detail {
    ...
    RC_GTEST_PROP(unsigned_divmod, divide_then_multiply, (storage<128> dividend, storage<128> divisor)) {
        RC_PRE(!divisor.negative());
        RC_PRE(!dividend.negative());
        RC_PRE(divisor != make_storage(0));
    ...
    RC_GTEST_PROP(divmod, divide_then_multiply, (storage<256> dividend, storage<256> divisor)) {
        RC_PRE(divisor != make_storage(0));
    ...
    RC_GTEST_PROP(divmod, is_divide_and_remainder_64, (int64_t dividend, int64_t divisor)) {
        RC_PRE(divisor != 0);
    ...
    RC_GTEST_PROP(divmod, is_divide_and_remainder_32, (int32_t dividend, int32_t divisor)) {
        RC_PRE(divisor != 0);
