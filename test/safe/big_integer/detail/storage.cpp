...
namespace safe::_big_integer::detail {
    ...
    RC_GTEST_PROP(storage, get_set_is_consistent, (uint32_t value, storage<512> a)) {
    ...
    TEST(storage, equality) {
        ...
        ASSERT_EQ(storage<32>{{0x8000'0000u}}, (storage<64>{{0x8000'0000, 0xffff'ffffu}}));
        ASSERT_NE(storage<32>{{0x8000'0000u}}, (storage<64>{{0x8000'0000, 0x0000'0000u}}));
        ASSERT_EQ(storage<32>{{0x7000'0000u}}, (storage<64>{{0x7000'0000, 0x0000'0000u}}));
        ASSERT_NE(storage<32>{{0x7000'0000u}}, (storage<64>{{0x7000'0000, 0xffff'ffffu}}));
    }
