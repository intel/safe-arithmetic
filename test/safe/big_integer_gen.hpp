...
template <std::size_t NumBits> struct Arbitrary<safe_big_integer<NumBits>> {
    using T = safe_big_integer<NumBits>;
    static Gen<T> arbitrary() {
        using src = safe::_big_integer::detail::storage<NumBits>;

        return gen::map(gen::arbitrary<src>(),
                        [](src const &v) { return T{v}; });
    }
};
} // namespace rc